...
import { zAssetReorder } from "@/lib/schemas";
import { reorderModuleAssets, COL } from "@/lib/firestore";
...
curl -X POST http://localhost:3000/api/admin/asset.reorder \
  ...
  -d '{
    ...
    "order": [
      ...
      {"assetId": "asset-3", "order": 1},
      {"assetId": "asset-2", "order": 2}
    ]
  }'
*/
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = zAssetReorder.parse(body);
    ...
    // Reorder assets
    await reorderModuleAssets(adminDb, parsed.moduleId, parsed.order);
    ...
    console.error("Asset reorder error:", error);
